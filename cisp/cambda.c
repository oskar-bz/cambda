...
cbType get_replacement(SubstSet *s, u64 temp_id) {
  for (int i = 0; i < s->used; i++) {
    cbSubst *s = olArray_get((olArray*)s, i);
...
SubstSet unify(cbState *ctx, cbType a, cbType b, cbError *out_error) {
  *out_error = cbError_ok();
  ...
  if (b.is_temp) {
    ...
    // a is a concrete type and not a function, b is a concrete type or a
    // function
...
cbType instantiate(cbState *ctx, cbScheme *scheme) {
  olArray_of(u64) replace_with =
      olArray_make(sizeof(u64), scheme->foralls.used);
...
SubstSet infer(cbState *ctx, olMap *type_env, cbExpression *expr,
               cbType *out_type, cbError *out_error);

SubstSet infer_fn(cbState *ctx, olMap *type_env, cbExpression *expr,
                  cbType *out_type, cbError *out_error) {
  *out_error = cbError_ok();
  for (int i = 0; i < expr->lit.fn_->args.used; ++i) {
    cbArg *arg = olArray_get(&expr->lit.fn_->args, i);
    cbType arg_type =
        (cbType){.is_temp = true, .is_fn = false, .id = ctx->cur_temp++};
  ...
  SubstSet substitutions =
      infer(ctx, type_env, expr->lit.fn_->ebody, &return_type, out_error);
...
SubstSet infer_app(cbState *ctx, olMap *type_env, cbExpression *expr,
                   cbType *out_type, cbError *out_error) {
  cbType return_type = (cbType){.is_temp = true, .is_fn = false};
  cbType fn_type;
  SubstSet return_subst;
  ...
  // infer right hand types for every supplied argument
  for (int i = 0; i < expr->app.supplied_args.used; ++i) {
    ...
    return_subst = subst_union(&substs, &sub2);
    cbType fn_type_sub = apply(&sub2, fn_type);
    cbType type3 = type_make(false, true, type2, return_type);
    
    SubstSet sub3 = unify(ctx, fn_type_sub, type3, out_error);
    return_subst = subst_union(&return_subst, &sub3);
    if (out_error->kind != ERROR_OK) {
      return NULLSUBSTSET;
    }
  }
  ...
  *out_error = cbError_ok();
  ...
  case EVAR: {
    ...
    if (scheme == null) {
      ...
      *out_error = errorf(ERROR_UNDEFINED_VAR, expr->loc, "Undefined var '%s'",
                          varname ? varname->data : "<not found>");
      return NULLSUBSTSET;
    }
