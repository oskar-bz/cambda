...
/* Generalization is implicit and FREE: a type is a "scheme" paired with a
 * binding level `glvl`; any variable whose level exceeds glvl is generic (to be
 * freshened on instantiation). There is no generalize pass and no separate
 * quantified-node kind — a `let` just records the current level. This is the
 * standard level/rank trick (a la OCaml) and removes a whole tree rewrite that
 * every let used to perform.
 *
 * Persistent (top-level) schemes outlive the per-statement scratch reset, so
 * they are deep-copied into the perm arena, preserving variable levels. Nested
 * (let-local) schemes are used within the same statement and need no copy —
 * sharing their free variables with the live type graph is in fact required. */
static Type *copy_perm_rec(Type *t) {
    t = prune(t);
    if (t->stamp == g_stamp) return t->memo;
    Type *r;
    switch (t->kind) {
        case T_VAR:   r = mk(T_VAR); r->var.id = t->var.id; r->var.level = t->var.level; break;
        case T_ARROW: { Type *f = copy_perm_rec(t->arrow.from), *g = copy_perm_rec(t->arrow.to); r = mk_arrow(f, g); break; }
        default:      r = t; break;   /* T_CON already lives in perm */
    }
    t->stamp = g_stamp; t->memo = r;
    return r;
}
static Type *copy_perm(Type *t) {
    Arena *save = g_TA; g_TA = &g_perm; ++g_stamp;
    Type *r = copy_perm_rec(t);
    g_TA = save;
    return r;
}
...
/* Memoized + copy-free: a generic var (level > g_instlvl) becomes a fresh var;
 * a free/monomorphic var or constant returns the original node (zero alloc and,
 * importantly, keeps sharing with the live type graph); arrow subtrees are
 * copied only if something below changed. */
static int g_instlvl;
static Type *inst_rec(Type *t, int *changed) {
    switch (t->kind) {
        case T_LINK: return inst_rec(prune_slow(t), changed);
        case T_VAR:
            if (t->var.level > g_instlvl) { *changed = 1; return imap_get(t->var.id); }
            return t;
        case T_ARROW:
        ...
        default: return t;                        /* T_CON */
    }
}
static Type *instantiate(Type *scheme, int glvl) {
    ++g_stamp; imap_reset(); g_instlvl = glvl; int c = 0;
...
static int    *g_gschemelvl;      /* generic threshold per global scheme */
...
static int global_define(Sym name) {
    ...
    if (g_nglob == g_capglob) {
        ...
        g_gschemelvl = realloc(g_gschemelvl, g_capglob*sizeof *g_gschemelvl);
        g_gvalue     = realloc(g_gvalue,     g_capglob*sizeof *g_gvalue);
    }
    s = g_nglob++;
    g_gname[s] = name; g_gscheme[s] = NULL; g_gschemelvl[s] = 0; g_gvalue[s] = NULL;
    return s;
}
...
/* ===================================================================== */
typedef struct { Type *ty; int glvl; int poly; } Local;  /* glvl: generic threshold */
...
static Type *infer(Expr *e) {
    switch (e->kind) {
        ...
        case E_LOCAL: {
            ...
            return L.poly ? instantiate(L.ty, L.glvl) : L.ty;
        }
        case E_GLOBAL: return instantiate(g_gscheme[e->v.idx], g_gschemelvl[e->v.idx]);
        case E_LAM: {
        ...
        case E_LET: {
            ...
            if (e->let.rec) {
                ...
                unify(tv, t1);
                exit_level();
                g_locals[p].ty = t1; g_locals[p].glvl = g_level; g_locals[p].poly = 1;  /* re-bind for body */
            } else {
                enter_level();
                Type *t1 = infer(e->let.rhs);
                exit_level();
                g_locals[p].ty = t1; g_locals[p].glvl = g_level; g_locals[p].poly = 1; g_nl++;
            }
...
/* infer a closed expression; its free vars print as a, b, c ... directly */
...
static void exec_binding(int rec, Sym name, Expr *rhs) {
    ...
    if (rec) {
        ...
        g_gschemelvl[slot] = g_level;    /* threshold = entered level, so tv is not generic */
        t1 = infer(rhs);
        unify(tv, t1);
    } else {
        t1 = infer(rhs);
    ...
    exit_level();
    /* persistent scheme: deep-copy into perm, generic threshold = current level */
    Type *scheme = copy_perm(t1);
    if (!rec) slot = global_define(name);
    g_gscheme[slot] = scheme;
    g_gschemelvl[slot] = g_level;
